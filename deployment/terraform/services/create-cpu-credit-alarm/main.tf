...
resource "aws_lambda_function" "create_cpu_credit_alarm_lambda_function" {
  function_name    = local.name-prefix
  description      = "This lambda program creates cpu credit alarm for instance of T class."
...
resource "aws_cloudwatch_log_group" "cloudwatch_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.create_cpu_credit_alarm_lambda_function.function_name}"
  retention_in_days = var.logs-retention-days
}


resource "aws_cloudwatch_event_rule" "create_cpu_credit_alarm_event_rule" {
  name           = local.name-prefix
  description    = "Create cpu credit balance alarm."
...
resource "aws_cloudwatch_event_target" "create_cpu_credit_alarm_rule_target" {
  event_bus_name = var.ec2-event-bus-name
  rule           = aws_cloudwatch_event_rule.create_cpu_credit_alarm_event_rule.name
  arn            = aws_lambda_function.create_cpu_credit_alarm_lambda_function.arn
}
...
resource "aws_lambda_permission" "lambda_permission_for_lambda" {
  ...
  function_name = aws_lambda_function.create_cpu_credit_alarm_lambda_function.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.create_cpu_credit_alarm_event_rule.arn
}

output "function_arn" {
  value = aws_lambda_function.create_cpu_credit_alarm_lambda_function.arn
}

output "function_name" {
  value = aws_lambda_function.create_cpu_credit_alarm_lambda_function.function_name
}